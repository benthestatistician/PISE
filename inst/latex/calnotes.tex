...
\author{Ben B. Hansen\thanks{This work benefited from helpful comments of Jake Bowers, Joshua Errickson, Mark M. Fredrickson, and Xuming He, none of whom bear responsibility for potential errors or omissions.}}
...
index coefficients themselves, but larger:

\begin{equation*}
|(\vec{x}_i -\vec{x}_j)(\hat\beta - \betatrue)| \leq |\vec{x}_i -\vec{x}_j|_2 |\hat\beta - \betatrue|_2 = O(p^{1/2})O_P[\sqrt{p/n}] = O_P(p/\sqrt{n}) . 
\end{equation*}

Even among pairs $(i,j)$ that are perfectly matched on the index score,
$(\vec{x}_i - \vec{x}_j)\betatrue = 0$, there are still $p-1$ nonzero
components in the sum that is $|\vec{x}_i - \vec{x}_j|_2^2$, and
differences on the estimated index,
$|(\vec{x}_i - \vec{x}_j)\hat\beta|_2$, are of order
$(p-1)^{1/2} \cdot (p/n)^{1/2}$, i.e.~again $p/\sqrt{n}$.
...
\section{The range of index score models considered} \label{sec:range-prop-score}
...
The method being developed here will require $p^2/n \downarrow 0$, as indicated in Section~\ref{sec:introduction}; but most of the supporting theory is valid under the weaker \ref{A-rates}.
...
Our regularity conditions entail consistency of $\hat\beta$, and of common estimators of $C_{n}$.  If $\psi$ is the gradient a (well-specified) log-likelihood, then $A_{n}=B_{n}$, and $C_{n}$ is estimated by $n^{-1}\hat{A}_{n}^{-1}(\hat\beta)$; more broadly, it is estimated by 
% \begin{align*}
...
Let \ref{A-estimable}--\ref{A-rates} hold,  so that $\hat\beta$ and $\hat{C}$ are consistent estimates of $\betatrue$ and $C$: $|\hat\beta - \betatrue|_{2}^{2} = O_{P}\left(\frac {p}{n}\right)$ \marginpar{ToDo: confirm proof doesn't really assume $o_{P}\left(\frac {p}{n}\right)$} \citep[Theorem~2.1 and Example~3]{he2000parameters}; $n|\hat{C} - C|_{2} \stackrel{P}{\rightarrow} 0$ (see Section~\ref{sec:range-prop-score}??).  Then  %For sequences $\{d_{n} \} \subseteq \Re^{+}$, $\mathcal{D}_{n} = \{ \vec{x}_{i} - \vec{x}_{j}: |\vec{x}_{i} - \vec{x}_{j}|_{2} \leq d_{n}\}$,
\begin{equation*}
...
termed \textit{paired index standard error} (PISE).  (Or if the is a
