...
% \item \label{A-l2Sfinite} The following quantities are $O(n)$:  $|(n-1)S^{(x)}|_{2} =$ 
...
% In light of this, the old A-l2Sfinite was redundant; see also notes
% to git commit titled "In light of $|x|_\infty$ condition, excise |X|_2 moment conditions"
% A-boundedXes is used to prove Lemma~\ref{lem:ChatC}
%in at least four places, none of which seems totally
...
\begin{proof}[Proof of Lemma~\ref{lem:C-rate}]
  In light of \eqref{eq:13}, Conditions~\ref{A-psismooth} and~\ref{A-boundedXes}%\ref{A-l2Sfinite}
...
The $|\hat\beta - \betatrue|_{2}$-consistency component of the lemma follows from Conditions~\ref{A-estimable}, \ref{A-boundedXes}%\ref{A-l2Sfinite}
...
% Similarly $|S^{(x)}\betatrue|_{2} = O(??)$, due to \ref{A-Sfullrank}. \marginpar{Conj: $\betatrue'S^{(x)}\betatrue \sim O(\log p ) \Rightarrow |S^{(x)}\betatrue| \sim O(\log p)$.} 
\begin{align*}
