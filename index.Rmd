...
>    - Excludes treatment subjects whose $\mathbf{x}\hat\beta$ has larger-than-chance separation from controls' $\mathbf{x}\hat\beta$s.    
>- Assume well-conditioned g.l.m., dimension $p = o(n/\log (n))$.  (So that $\hat\beta - \beta \stackrel{P}{\rightarrow} 0$.)  
>- If $\mathbf{d} = \mathbf{x}_1 - \mathbf{x}_2$, $\mathbf{d}(\hat{\beta} - \beta)$ is the error of estimation of paired difference $\mathbf{d}\beta$.
...
>- Also **all possible** pairs include many that are quite different on $\mathbf{x}\beta$.  We wanted to characterize $\mathbf{E} (\mathbf{d}_r(\hat{\beta} - \beta))^2$ among pairs $r$ s.t. $\mathbf{d}_r\beta \approx 0$.  
