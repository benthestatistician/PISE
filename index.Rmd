...
>- In large studies, PS may be estimable w/ more precision than $.25s_p(\widehat{\mathrm{PS}})$.  At same time, more potential controls. 
...
(\hat\beta - \beta)' \left[\frac{1}{\nu}
                   \sum_{i \lesssim j} (\mathbf{x}_i - \mathbf{x}_j)'(\mathbf{x}_i - \mathbf{x}_j)\right] (\hat\beta - \beta)\\
		   &=:(\hat\beta - \beta)' (2 S^{(p)} )
(\hat\beta - \beta).
\end{align*}		   
\]
...
- Diminishing caliper calculations tend to bring this problem to light. Spectral condition numbers & index standard errors ($\sqrt{\langle \hat{S}^\perp, \hat{C}_\hat\beta\rangle_F}$) for 3 fits of the same logistic regression:
