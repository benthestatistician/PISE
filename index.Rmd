...
>- Assume well-conditioned g.l.m., dimension $p = o(n/\log (n))$.  (So that $\hat\beta - \beta \stackrel{P}{\rightarrow} 0$.)  
...
>- As $\mathrm{Cov}(\hat\beta)$ is hard to estimate directly, consider $\mathrm{Cov}(\tilde\beta)$, $\tilde\beta = \beta + n^{-1}\sum_{i=1}^{n}\mathrm{IC}({Z}_{i}, \mathbf{x}_{i}; \beta)$.
>- Also **all possible** pairs include many that are quite different on the PS.  We wanted to characterize $\mathbf{E} (\mathbf{d}_r(\hat{\beta} - \beta))^2$ among pairs $r$ s.t. $\mathbf{d}_r\beta \approx 0$.  
...
>- Rather than the mean-square of PS-difference estimation errors, ${n \choose 2}^{-1} \mathbf{E} \sum_r (\mathbf{d}_r(\hat{\beta} - \beta))^2$, consider a mean-square in $\tilde\beta$, net of differences in the true PS: 
\[
...
>- Plug in $\hat{S}^\perp = \widehat{\mathrm{Cov}}(\mathbf{x}^{\perp \mathbf{x}\hat{\beta}})$, $\hat{C}_\hat\beta$  from the regression fit to give the "paired index standard error" (PISE).
...
>- Here 2.5 PISEs works out to 4 logits ($3.9s_p$).
...
>- In that case, any $w_n$ that's $O_P(p/n^{1/2})$ will do the trick.  E.g.,  $w_n \equiv 2.5\text{PISE}$.

---

### Discussion

>- Roughly speaking, the PISE is the r.m.s. of $\mathbf{x}_i\hat\beta-\mathbf{x}_j\hat\beta$ among pairs $(i,j)$ s.t. $\mathbf{x}_i\beta = \mathbf{x}_j\beta$. 
>- If $p^2/n \downarrow 0$, then PISE $\stackrel{P}{\rightarrow} 0$, and matching within $k$ PISEs suffices for $\max_{i\sim j} |\mathbf{x}_i\beta-\mathbf{x}_j\beta| \stackrel{P}{\rightarrow} 0$.
...
>- If your matches are farther than a few PISEs, you're matching outside region of overlap. 
>- I like calipers of $k=2.5$ PISEs.


<!-- Matches between 1 and 3 PISEs might be considered "marginal" (Austin & Lee, 2009).-->
