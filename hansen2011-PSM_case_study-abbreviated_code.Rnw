...
in a prognostic propensity score full match, as discussed and recommended in my paper ``Propensity score
matching to extract latent experiments from nonexperimental data: A
case study''  \citep{hansen2009b}.  The longer, unsimplified session is
...
<<>>=
load("~/Documents/data/satcoach/satcoach.RData")
library(optmatch)
library(RItools)
library(xtable)
library(splines)
...
<<>>=
###as of v 0.8-5, replaces subseq 2 lines
...
<<>>=
###as of v 0.8-5, next 4 lines replaced by subseq 3 lines
###pg.ppty$within <- exactMatch(Coach ~ ethn3levels + dadsed, data=satcoach)
###pg.ppty$dist <- match_on(pg.ppty$model, within=pg.ppty$within, data=satcoach)
###pg.ppty$dist.with.ppty.calipers <- pg.ppty$dist + caliper(ppty$dist, width=0.5)
##fm.pgppty.pptyclpr <- fullmatch(pg.ppty$dist.with.ppty.calipers,data=satcoach)

pg.ppty$within <- exactMatch(Coach ~ ethn3levels + dadsed, data=satcoach)
pg.ppty$within <-  match_on(ppty$model, within=pg.ppty$within, caliper=.5)
fm.pgppty.pptyclpr <- fullmatch(pg.ppty$model, within=pg.ppty$within, data=satcoach)
###previous replaces commented lines prior, obviating need for 
###pg.ppty$dist and pg.ppty$dist.with.ppty.calipers and fm.pgppty.pptyclpr
...
plot(xb, legend=FALSE, 
     xlab="(Adjusted coached minus uncoached difference)/(pooled sd)")
legend(x="topright", 
...
\bibitem[{Hansen(2009)}]{hansen2009}
--- (2009), \enquote{Propensity score matching to recover latent experiments:
  diagnostics and asymptotics,} Tech. Rep. 486, Statistics Department,
  University of Michigan.

\bibitem[{Hansen(2011)}]{hansen2009b}
--- (2011), \enquote{Propensity score matching to extract latent experiments
  from nonexperimental data: A case study,} in \textit{Looking Back:
  Proceedings of a Conference in Honor of Paul W. Holland}, eds. Dorans, N. and
  Sinharay, S., Springer, chap.~9, pp. 149--181.
...
Rosenbaum, P.~R. (2002), \enquote{Covariance adjustment in randomized
  experiments and observational studies,} \textit{Statistical Science}, 17,
  286--327.
