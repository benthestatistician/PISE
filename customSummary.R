### DEPENDENCIES: matchedDifferencesFromOptmatch.R (as of R265).
###               sandwich_glm.R (as of R298)
### (earlier dep on matchedPSdistances.R has now been removed.)
...
###  so$matched.dist.quantiles <- quantile(matchdists, prob=quantiles)
    if(!is.null(propensity.model) &&
     inherits(propensity.model, "glm"))
      {
        so$standardizer <- if ('standardization.scale' %in% names(cl)) cl[["standardization.scale"]] else "mad"
        so$PS.pooled.sd <- optmatch:::szn.scale(propensity.model$linear.predictors,
                                                propensity.model$y>0,
                                                standardization.scale)
        so$PS.diffs <- PSdiffs(propensity.model, object, covariance.extractor=SandwichVcov) #from matchedDifferencesFromOptmatch.R
      }
...
###  print(signif(x$matched.dist.quantiles, digits))
  ...
  if ('PS.diffs' %in% names(x))
    { ## At the end of the day maybe only one of the below belongs -- not sure.

      cat("Percentiles of |matched (linear) PS diff|/s_p(linear PS):\n")
        print(signif(with(x$PS.diffs, quantile(abs(diff), prob=quantiles))/x$PS.pooled.sd, digits))
        cat("Percentiles of |matched PS diff|/SE(PS diff):\n")
        print(signif(with(x$PS.diffs,
                          quantile(ifelse(SE==0,
                                          ifelse(diff==0,0,Inf),
                                          abs(diff)/SE),
                                   prob=quantiles)), digits))
      cat("s_p(linear PS) = ", signif(x$PS.pooled.sd, digits),
          "; Percentiles of SE(linear PS diff):\n")
      print(signif(quantile(x$PS.diffs$SE, prob=seq(0,1,len=5)), digits=digits))
      cat(paste("(s_p(linear PS) calculated SDs using ", x$standardizer,"(), prior to pooling.)\n", sep=""))
    }
